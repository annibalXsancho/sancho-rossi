...
- **Angles adoucis** : tokens `--radius-sm` 6px (marqueurs, jauges, pastilles < 30 px) / `--radius` 10px (contrôles) / `--radius-lg` 16px (cartes, panneaux) / `--radius-full` (chips, pills, recherche). Espacement sur l'échelle `--sp-1..5`, **durées sur `--t-fast` 120 / `--t` 150 / `--t-slow` 220 ms, toujours en `ease-out`** (S12) — ne plus écrire de durée en dur.
- **Échelle de difficulté : neutre → ambre → rouge** *(tranché avec l'utilisateur le 29/07/2026, S12)*. « Facile » = tonal neutre (`--surface-2`/`--text-soft`), « modéré » = ambre `#d99a3d` (la teinte d'alerte déjà portée par `.pill-warn`), « difficile » = rouge. Le feu tricolore n'est **pas** assumé : **aucun vert dans l'app**, et la couleur ne sert qu'à signaler ce qui demande de l'attention. Même raison pour le bleu-gris de la pastille GPX, neutralisé.
- **Contrôles de formulaire** : `select`/`input` sans cadre, remplissage `--surface-2`, chevron maison en data-URI (S12) — ne jamais reposer un `border: 1px` autour d'un contrôle. Anneau de focus unique via la règle globale `:focus-visible` (`--focus-ring`).
...
- **Fluide** : transitions/micro-animations discrètes (tokens `--t-*`, ease-out), jamais de saut de layout, états de chargement élégants (squelettes, pas de spinners bruts). Emoji d'un bouton ou d'un chip : dans un `<span class="btn-ic" aria-hidden>` à part, jamais collé au libellé.
