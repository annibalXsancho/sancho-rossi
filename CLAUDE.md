...
- **Échelle sémantique** *(tranchée en S12, 01/08/2026)* : `--warn` (ambre) et `--info` (ardoise) sont les deux seules couleurs hors accent, en variables — plus aucun `#hex` de couleur sémantique dans le CSS. La **difficulté se lit neutre → ambre → rouge** : « facile » est NEUTRE (absence d'alerte), pas verte. Le feu tricolore est écarté ; ne pas réintroduire de vert.
- **Icônes dessinées, jamais d'emoji dans la chrome** : boutons, titres, états vides et sélecteurs prennent des SVG au trait en `currentColor` (`LAYER_ICONS` dans map.js, `EMPTY_ICONS` dans empty.js). Les emojis ne subsistent que dans le **contenu** — pictogrammes météo, types de repères de terrain, phases de lune, et les messages envoyés aux proches.
- **États vides** : une seule primitive, `emptyState()` (`js/empty.js`) — icône dessinée + une phrase + l'action qui débloque. Jamais un `<p>Aucun…</p>` ad hoc.
